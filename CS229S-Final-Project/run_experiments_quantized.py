...
def train(dataset='wikitext', batch_size=4, max_iters=500, block_size=1024, gradient_accumulation_steps=40):
    ...
    while True:
        ...
        if iter_num > max_iters:
            print("Break out of training......")
            break

    throughput = iter_num * len(Y) / (time.time() - t0)

    return throughput, lossf, model
...
def inference(model, batch_size, max_new_tokens, temperature, top_k, device, dtype, num_samples=1):
    ...
    with torch.no_grad():
        ...
        for _ in range(num_samples):
            ...
            if logits.size(1) > Y.size(1):
                ...
                print("logits new", logits.size(1))
            perplexity = model.perplexity_score(logits, Y)
            # y = torch.tensor(y, dtype=torch.int64).cuda()
            # Y = torch.tensor(Y, dtype=torch.int64).cuda()
            # perplexity = model.perplexity_score(y, Y)
    ...
    return tokens_per_sec, perplexity
...
def main():
    _, loss, model = train(dataset='wikitext', batch_size=4, max_iters=1, block_size=1024, gradient_accumulation_steps=40)
    ...
    latency_1, perplexity = inference(model, 1, 10, 0.8, 200, torch.device("cuda"), 'bfloat16', 5)
