...
# I/O
out_dir = 'structured_out' # unstructured_out
finetune_dir = 'structured_finetune' # unstructured_finetune
eval_interval = 50
log_interval = 1
eval_iters = 200
...
# -----------------------------------------------------------------------------
PRUNING_METHOD = "unstructured" # "unstructured"
# various inits, derived attributes, I/O setup
ddp = int(os.environ.get('RANK', -1)) != -1 # is this a ddp run?

prune_dict_structured = {}
prune_dict_unstructured = {}
...
def build_structured_mask(weight_index_list, weight):
...
def build_unstructured_mask(weights, pruning_rate):
    weights = weights.reshape((1, -1))
    # quantile_value = torch.quantile(weights, pruning_rate, dim=1)
    quantile_value = approximate_quantile(weights, pruning_rate)
    mask = (weights > quantile_value).float()
...
def unstructured_pruning(model, pruning_rate):
    for name, module in model.named_modules():
        ...
        if isinstance(module, torch.nn.Linear):
            ...
            mask = build_unstructured_mask(weights, pruning_rate)
            mask = mask.reshape(weights.shape)
            # print("mask shape: ", mask.shape)
            assert weights.shape == mask.shape, "unstructurd pruning shape not match"
            module.weight.data.mul_(mask)
            prune_dict_unstructured[name] = mask
            # print("pruning weight", module.weight.data)

        # LayerNorm
        elif isinstance(module, LayerNorm):
            weights = torch.abs(module.weight.data.reshape((1, -1)))
            # print("weights shape", weights.shape)
            mask = build_unstructured_mask(weights, pruning_rate)
            mask = mask.reshape(weights.shape)
...
def structured_pruning(model, pruning_rate):
    for name, module in model.named_modules():
        ...
        if isinstance(module, torch.nn.Linear):
            ...
            prune_dict_structured[name] = weight_index_list

            mask = build_structured_mask(weight_index_list, weights).to(device)
...
def finetune(model, dataset='wikitext', batch_size=4, max_iters=100, block_size=1024, gradient_accumulation_steps=40):
    for name, module in model.named_modules():
        if PRUNING_METHOD == "structured":
            if isinstance(module, torch.nn.Linear):
                weight_index_list = prune_dict_structured[name]
                mask = build_structured_mask(weight_index_list, module.weight.data).to(device)
                module.weight.data.mul_(mask)
            elif isinstance(module, LayerNorm):
                weight_index_list = prune_dict_structured[name]
                mask = build_structured_mask(weight_index_list, module.weight.data.reshape((1, -1))).to(device)
                module.weight.data.mul_(torch.squeeze(mask))
        else:
            if isinstance(module, torch.nn.Linear):
                mask = prune_dict_unstructured[name].to(device)
                module.weight.data.mul_(mask)
            elif isinstance(module, LayerNorm):
                mask = prune_dict_unstructured[name].to(device)
                module.weight.data.mul_(torch.squeeze(mask))
...
results = {}
pruning_rates = [0.9, 0.7,0.5, 0.3, 0.1]

for pruning_rate in pruning_rates:
    results[pruning_rate] = {}

    total_time = 0
    if PRUNING_METHOD == "structured":
        start_time = time.time()
        pruned_model = structured_pruning(orign_model, pruning_rate)
        end_time = time.time()
        total_time = end_time - start_time

        # print("Total time for structured pruning:", total_time)
    else:
        start_time = time.time()
        pruned_model = unstructured_pruning(orign_model, pruning_rate)
        end_time = time.time()
        total_time = end_time - start_time
    
    finetuned_model, throughput, lossf = finetune(pruned_model, dataset='wikitext', batch_size=4, max_iters=100, block_size=1024, gradient_accumulation_steps=40)
    print("pruning rate: ", pruning_rate)
    results[pruning_rate]["lossf"] = lossf
    print("Total time for structured pruning:", total_time)
    results[pruning_rate]["total_time"] = total_time

with open('results_unstructured.json', 'w') as fp:
