...
class BertSTS(torch.nn.Module):
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, sent_1, sent_2, device):
        ...
        sentence_1_real = torch.pow(torch.fft.fft(sentence_1).real,2)
        sentence_2_real = torch.pow(torch.fft.fft(sentence_2).real,2)
        sentence_1_img = torch.pow(torch.fft.fft(sentence_1).imag,2)
        sentence_2_img = torch.pow(torch.fft.fft(sentence_2).imag,2)
        sentence_1 = torch.sqrt(sentence_1_real + sentence_1_img)
        sentence_2 = torch.sqrt(sentence_2_real + sentence_2_img)
