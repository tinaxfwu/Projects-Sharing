...
class BertSTS(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer = torch.nn.Linear(300, 150, bias = True)
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, sent_1, sent_2, device):
        ...
        #sentence_1 = self.linear_layer(self.activation(torch.fft.fft(sentence_1).real))
        #sentence_2 = self.linear_layer(self.activation(torch.fft.fft(sentence_2).real))
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            logits = logits * 5
            loss = F.mse_loss(logits, labels.view(-1).float())/args.batch_size
            #print(loss)
            loss.backward()
            optimizer.step()
