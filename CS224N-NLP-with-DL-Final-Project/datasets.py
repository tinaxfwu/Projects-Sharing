...
class RandomMaskDataset(Dataset):
    ...
    def pad_data(self, data):
        # I a very happy 
        sents = [x[0] for x in data] # sentence with [MASK] : "I am [MASK] happy"
        sent_ids = [x[2] for x in data]
        predict = [x[3] for x in data] # "very"
        encoding = self.tokenizer(sents, return_tensors='pt', padding=True, truncation=True) 
        encoding2 = self.tokenizer(predict, return_tensors='pt', padding=True, truncation=True)

        token_ids = torch.LongTensor(encoding['input_ids'])
        attention_mask = torch.LongTensor(encoding['attention_mask'])
        label = torch.LongTensor(encoding2['input_ids'])
        attention_mask_label = torch.LongTensor(encoding2['attention_mask'])

        return token_ids, attention_mask, label, attention_mask_label, sents, sent_ids

    
    def collate_fn(self, all_data):
        token_ids, attention_mask, label, attention_mask_label, sents, sent_ids = self.pad_data(all_data)
...
class SentencePairDatasetModified(Dataset):
...
class SentencePairTestDatasetModified(Dataset):
    def __init__(self, dataset, args):
        self.dataset = dataset
        self.p = args
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
