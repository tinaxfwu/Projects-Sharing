...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # high: correspond to the start of the DCT output array
        self.freq_filters_low_pass = nn.Parameter(torch.tensor(np.array([0]*int(config.hidden_size*2/3) + [1]*int(config.hidden_size/3)), dtype=torch.float))
    ...
    def apply_dct_filter(self, x, filter):
        ...
        x_freq = dct.dct(x.cpu().detach().numpy(), type=2, axis=-1, norm='ortho')
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        original_dct = self.apply_dct_filter(original, self.freq_filters_low_pass) #768 -> 768
        cat = self.dropout(original_dct)
    ...
    def predict_paraphrase(self,
        ...
        sentence_1_dct = self.apply_dct_filter(sentence_1, self.freq_filters_low_pass)
    
        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2))
        sentence_2_dct = self.apply_dct_filter(sentence_2, self.freq_filters_low_pass)
        ...
        sentence_cat = self.relu(sentence_cat).squeeze(dim = 1)
        return sentence_cat
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter_low.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter_low")
