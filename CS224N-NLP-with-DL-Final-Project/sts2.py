...
class BertSTS(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.lstm_cell = torch.nn.LSTM(config.hidden_size, 100, 2)
        self.config = config
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, sent_1, sent_2, device):
        ...
        sentence_1, _ = self.lstm_cell(sentence_1)
        sentence_2 = self.dropout(self.bert(input_ids_2, attention_mask_2)["last_hidden_state"])
        sentence_2, _ = self.lstm_cell(sentence_2)

        sentence_1 = sentence_1.mean(dim=1)
        sentence_2 = sentence_2.mean(dim=1)
