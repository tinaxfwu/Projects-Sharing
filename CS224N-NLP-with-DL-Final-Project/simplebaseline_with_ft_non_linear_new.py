...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer_sentiment = torch.nn.Linear(config.hidden_size * 2, 5, bias = True)
        self.linear_layer_paraphrase = torch.nn.Linear(config.hidden_size * 4, 1, bias = True)
        self.linear_layer_sts= torch.nn.Linear(config.hidden_size * 4, 1, bias = True)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        return self.relu(self.linear_layer_sentiment(cat))
    ...
    def predict_paraphrase(self,
        ...
        sentence_1_fft = torch.fft.fft(sentence_1).real
        sentence_1 = self.dropout(torch.cat((sentence_1, sentence_1_fft),1))
        
        sentence_2 = self.forward(input_ids_2, attention_mask_2)
        sentence_2_fft = torch.fft.fft(sentence_2).real
        sentence_2 = torch.cat((sentence_2, sentence_2_fft),1)

        sentence_cat = torch.cat((sentence_1, sentence_2), 1)
        return self.relu(self.linear_layer_paraphrase(sentence_cat)).squeeze(dim = 1)
    ...
    def predict_similarity(self,
        ...
        sentence_cat = torch.cat((sentence_1, sentence_2), 1)
        return self.relu(self.linear_layer_sts(sentence_cat)).squeeze(dim = 1)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft_3_non_linear_new.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft_3_non_linear_new")
