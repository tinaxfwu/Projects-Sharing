...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.filter_prp1 = nn.Parameter(torch.rand(config.hidden_size, dtype=torch.float))
        self.filter_prp2 = nn.Parameter(torch.rand(config.hidden_size, dtype=torch.float))
    ...
    def predict_paraphrase(self,
        ...
        sentence_1 = self.dropout(self.forward(input_ids_1, attention_mask_1, self.filter_prp1))
        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2, self.filter_prp2))
    ...
    def predict_similarity(self,
                           input_ids_1, attention_mask_1,
                           input_ids_2, attention_mask_2):
        sentence_1 = self.dropout(self.forward(input_ids_1, attention_mask_1, self.filter_sts1))
        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2, self.filter_sts2))
        sentence_cat = torch.cat((sentence_1, sentence_2),1)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simplebaseline_with_dct_only.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simplebaseline_with_dct_only")
