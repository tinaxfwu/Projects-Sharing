...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer_sentiment_1 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
        self.linear_layer_sentiment_2 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
        self.linear_layer_sentiment = torch.nn.Linear(config.hidden_size, 5, bias = True)
        self.linear_layer_paraphrase_1 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
        self.linear_layer_paraphrase_2 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
        self.linear_layer_paraphrase = torch.nn.Linear(config.hidden_size * 2, 1, bias = True)
        self.linear_layer_sts_1 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
        self.linear_layer_sts_2 = torch.nn.Linear(config.hidden_size, int(0.5 * config.hidden_size), bias = True)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        original_fft = self.relu(torch.fft.fft(original).real)
        cat = torch.cat((self.linear_layer_sentiment_1(original), self.linear_layer_sentiment_2(original_fft)),1)
    ...
    def predict_paraphrase(self,
        ...
        sentence_1_fft = self.relu(torch.fft.fft(sentence_1).real)
        sentence_1 = torch.cat((self.linear_layer_paraphrase_1(sentence_1), self.linear_layer_paraphrase_2(sentence_1_fft)),1)

        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2))
        sentence_2_fft = self.relu(torch.fft.fft(sentence_2).real)
        sentence_2 = torch.cat((self.linear_layer_paraphrase_1(sentence_2), self.linear_layer_paraphrase_2(sentence_2_fft)),1)
    ...
    def predict_similarity(self,
        ...
        sentence_1 = self.dropout(self.forward(input_ids_1, attention_mask_1))
        sentence_1_fft = self.relu(torch.fft.fft(sentence_1).real)
        sentence_1 = torch.cat((self.linear_layer_sts_1(sentence_1), self.linear_layer_sts_2(sentence_1_fft)),1)

        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2))
        sentence_2_fft = self.relu(torch.fft.fft(sentence_2).real)
        sentence_2 = torch.cat((self.linear_layer_sts_1(sentence_2), self.linear_layer_sts_2(sentence_2_fft)),1)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft_3_non_linear.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft_3_non_linear")
