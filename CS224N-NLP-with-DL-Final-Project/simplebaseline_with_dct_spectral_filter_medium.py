...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.freq_filters_medium_pass = nn.Parameter(torch.tensor(np.array([0]*int(config.hidden_size/3) + [1]*int(config.hidden_size/3) + [0]*int(config.hidden_size/3)), dtype=torch.float))
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        original_dct = self.apply_dct_filter(original, self.freq_filters_medium_pass) #768 -> 768
    ...
    def predict_paraphrase(self,
        ...
        sentence_1_dct = self.apply_dct_filter(sentence_1, self.freq_filters_medium_pass)
    
        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2))
        sentence_2_dct = self.apply_dct_filter(sentence_2, self.freq_filters_medium_pass)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter_medium.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter_medium")
