...
class MultitaskBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        original_dct = torch.fft.fft(original).real
        # original_dct = original_dct * filter
        # cat = torch.cat((original, original_d
        return original_dct
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simplebaseline_with_fft_only.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simplebaseline_with_fft_only")
