...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer_dct_sentiment = torch.nn.Linear( config.hidden_size, 5, bias = True)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        cat = self.linear_layer_dct_sentiment(original_dct) # 768 -> 5
        cat = self.relu(cat)
    ...
    def predict_paraphrase(self,
        ...
        sentence_cat = torch.cat((sentence_1_dct, sentence_2_dct), 1) 
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_dct_spectral_filter")
