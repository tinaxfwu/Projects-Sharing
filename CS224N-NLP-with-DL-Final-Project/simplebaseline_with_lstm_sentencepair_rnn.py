...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rnn = torch.nn.RNN(config.hidden_size*2, config.hidden_size, 1, batch_first = True, bidirectional = True)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        last_hidden_fft = torch.fft.fft2(last_hidden).real
        combined = torch.cat((last_hidden, last_hidden_fft),2)
        _, output = self.rnn(combined)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_lstm_sentencepair_rnn.pt' # Save path.
    seed_everything(args.seed)  # Fix the seed for reproducibility.
    #wandb.init(project="multitask-bert",
               # config=args,
                #name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_lstm_sentencepair_rnn")
