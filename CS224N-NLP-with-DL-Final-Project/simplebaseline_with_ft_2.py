...
class MultitaskBERT(nn.Module):
    ...
    def predict_paraphrase(self,
        ...
        sentence_1 = torch.cat((sentence_1, torch.fft.fft(sentence_1).real),1)
        sentence_2 = self.dropout(self.forward(input_ids_2, attention_mask_2))
        sentence_2 = torch.cat((sentence_2, torch.fft.fft(sentence_2).real),1)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft2.pt' # Save path.
    seed_everything(args.seed) # Fix the seed for reproducibility.
    wandb.init(project="multitask-bert",
               config=args,
               name=f"{args.option}-{args.epochs}-{args.lr}-simple_baseline_with_ft2")
