...
class BertSTS(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer_1 = torch.nn.Linear(config.hidden_size +30, 100, bias = True)
        #self.linear_layer_2 = torch.nn.Linear(config.hidden_size + 30, config.hidden_size, bias = True)
        #self.linear_layer_3 = torch.nn.Linear(config.hidden_size * 2, 1, bias = True)
        self.dropout = torch.nn.Dropout(config.hidden_dropout_prob)
        self.config = config
        #self.loss = torch.nn.CosineEmbeddingLoss(reduction='none')
    ...
    def forward(self, input_ids_1, attention_mask_1, input_ids_2, attention_mask_2, sent_1, sent_2, device):
        ...
        for s in sent_1:
            ...
            sent_embedding_1.append(encoded_pos_tags)

        pos_embeddings_tensor_1 = torch.tensor(sent_embedding_1)
        pos_embeddings_tensor_1 = pos_embeddings_tensor_1.to(device)
        
        sentence_1 = torch.cat((sentence_1, pos_embeddings_tensor_1),1)
        sentence_1 = self.linear_layer_1(sentence_1)
        ...
        for s in sent_2:
            ...
            pos_tags = pos_tags[:30]
            encoded_pos_tags = [self.tag_to_ix.get(tag, 17) for tag in pos_tags]  # Use 12 for unknown tags
            sent_embedding_2.append(encoded_pos_tags)

        pos_embeddings_tensor_2 = torch.tensor(sent_embedding_2)
        pos_embeddings_tensor_2 = pos_embeddings_tensor_2.to(device)

        sentence_2 = torch.cat((sentence_2, pos_embeddings_tensor_2),1)
        sentence_2 = self.linear_layer_1(sentence_2)
        
        
        #obj = torch.tensor(1).unsqueeze(0)
        #obj = obj.to(device)
        #print(self.loss(sentence_1, sentence_2,obj))
        #print(self.loss(sentence_1, sentence_2,obj).shape)
        #combined = torch.cat((sentence_1, sentence_2), 1)
